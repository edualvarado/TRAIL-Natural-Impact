...
/// </summary>
public class Footprint : TerrainBrushFootprint
{
    #region Instance Fields

    [Header("Footprints Compression - SET UP")]
    public bool applyCompression = false;
    [Range(10000, 1000000)] public double youngMGround = 1000000;
    public float originalLengthZero = 0.3f;

    [Header("Footprints Compression Vegetation - SET UP")]
    [Range(10000, 10000000)] public double youngMVegetation = 1000000;
    ...
    [Header("Footprints Vertical Accumulation - SET UP")]
    public bool applyBump = false;
    [Range(0, 0.5f)] public float poissonR = 0.4f;

    [Header("Footprints Anisotropic Accumulation - SET UP")]
    public bool applyAnisotropy = false;
    [Range(0f, 1f)] public float beta = 0.4f;

    [Header("Footprints Anisotropic Accumulation - Debug")]
    ...
    public float stepStabilization = 0.0001f;
    public List<IEnumerator> stabilizationCoroutinesLeft = new List<IEnumerator>();
    public List<IEnumerator> stabilizationCoroutinesRight = new List<IEnumerator>();

    [Header("Stabilization - Debug")]
    ...
    public bool showStabilizationProcessLeft;
    public bool showStabilizationProcessRight;

    [Header("Contact Grid - SET UP")]
    [Range(0, 15)] public int gridSize = 10;
    [Range(0f, 1f)] public float raycastDistance = 0.075f;
    [Range(0f, 1f)] public float offsetRay = 0.04f;

    [Header("Contact Grid - Debug")]
    ...
    [Header("Contact Grid - Number of hits")]
    ...
    [Header("Contact Grid - Contact Area Feet-Ground")]
    ...
    [Header("Terrain Compression - Pressure")]
    ...
    public override void DrawFootprint(int xLeft, int zLeft, int xRight, int zRight)
    {
        ...
        // FILTER
        heightMapLeftFiltered = new float[2 * gridSize + 1, 2 * gridSize + 1];
        ...
        if (applyAnisotropy)
        {
            ...
            if (LeftHeelHeight < LeftToeHeight)
            {
                ...
                foreach (Vector3 pos in neighboursPositionsLeft)
                {
                    ...
                    float angle = Vector3.SignedAngle((pos - originContourLeft), TotalForceLeftTerrainProjection, Vector3.up);
            ...
            if (RightHeelHeight < RightToeHeight)
            {
                ...
                foreach (Vector3 pos in neighboursPositionsRight)
                {
                    ...
                    float angle = Vector3.SignedAngle((pos - originContourRight), TotalForceRightTerrainProjection, Vector3.up);
                    float cosineAngle = Mathf.Clamp(Mathf.Cos(angle * Mathf.Deg2Rad), 0f, 1f);
        ...
        // Bringing pressure to DeformTerrainMaster
        ...
        if (base.DeformationChoice == inputDeformation.useManualValuesWithVegetation)
        ...
        if (IsLeftFootGrounded && RunningCoroutineDeformationLeft == true)
        {
            ...
            StartCoroutine(IncreaseTerrainLeft(heightMapLeft, heightMapLeftBool, weightsBumpLeftInit, weightsBumpLeft, xLeft, zLeft));
        }
        else
        {
            // When the time is over or lift the foot, deformation stops
            StopCoroutine(DecreaseTerrainLeft(heightMapLeft, heightMapLeftBool, weightsBumpLeftInit, weightsBumpLeft, xLeft, zLeft));
            StopCoroutine(IncreaseTerrainLeft(heightMapLeft, heightMapLeftBool, weightsBumpLeftInit, weightsBumpLeft, xLeft, zLeft));
        }
        ...
        if (IsRightFootGrounded && RunningCoroutineDeformationRight == true)
        {
            if (!deformLeftFootOnly)
            {
                ...
                StartCoroutine(IncreaseTerrainRight(heightMapRight, heightMapRightBool, weightsBumpRightInit, weightsBumpRight, xRight, zRight));
            }
        }
        else
        {
            ...
            {
                // When the time is over or lift the foot, deformation stops
                StopCoroutine(DecreaseTerrainRight(heightMapRight, heightMapRightBool, weightsBumpRightInit, weightsBumpRight, xRight, zRight));
                StopCoroutine(IncreaseTerrainRight(heightMapRight, heightMapRightBool, weightsBumpRightInit, weightsBumpRight, xRight, zRight));
            }
        }
    ...
    /// <summary>
    /// Method for stabilizing the terrain after deformation.
    /// </summary>
    /// <param name="xLeft"></param>
    /// <param name="zLeft"></param>
    /// <param name="xRight"></param>
    ...
    public override void DrawStabilizeFootprint(int xLeft, int zLeft, int xRight, int zRight)
    {
        ...
        if (applyStabilization)
        {
            if (RunningCoroutineStabilizationLeft == true)
            {
                ...
                // TEST STABILIZATION
                //Debug.Log(" - Start Coroutine Stabilization FIRST LEFT");
                //stabilizationCoroutinesLeft.Add(StabilizationLeft(heightMapLeftWorld, cellsStabilizationDescendentFirstLeft));
                //StartCoroutine(stabilizationCoroutinesLeft[^1]);
            }
            else
            {
                // TEST STABILIZATION
                ...
                // TEST STABILIZATION
                //Debug.Log(" - Stop Coroutine Stabilization FIRST LEFT");
                //StopCoroutine(stabilizationCoroutinesLeft[^1]);
            }
            ...
            if (RunningCoroutineStabilizationRight == true)
            {
                if (!stabilizeLeftFootOnly)
                {
                    ...
                    // TEST STABILIZATION
                    //Debug.Log(" - Start Coroutine Stabilization FIRST RIGHT");
                    //stabilizationCoroutinesRight.Add(StabilizationRight(heightMapRightWorld, cellsStabilizationDescendentFirstRight));
                    //StartCoroutine(stabilizationCoroutinesRight[^1]);
                }
            }
            else
            {
                if (!stabilizeLeftFootOnly)
                {
                    ...
                    // TEST STABILIZATION
                    //Debug.Log(" - Stop Coroutine Stabilization FIRST RIGHT");
                    //StopCoroutine(stabilizationCoroutinesRight[^1]);
                }
            }
    ...
    public override void DrawGaussianSingle(int x, int z, float smoothStrength = 1, int smoothRadius = 1)
    {
        int adjustedRadius = (int)(gridSize * smoothStrength);

        Debug.Log("adjustedRadius: " + adjustedRadius);

        float[,] res = new float[2 * adjustedRadius + 1, 2 * adjustedRadius + 1];

        for (int zi = -adjustedRadius; zi <= adjustedRadius; zi++)
        {
            for (int xi = -adjustedRadius; xi <= adjustedRadius; xi++)
            {
                if (new Vector2(xi, zi).magnitude > adjustedRadius)
                {
                    res[zi + adjustedRadius, xi + adjustedRadius] = terrain.Get(x + xi, z + zi);
                }
                else
                {
                    float n = 2.0f * 1 + 1.0f;
                    float sum = 0;
                    for (int szi = -smoothRadius; szi <= smoothRadius; szi++)
                    {
                        for (int sxi = -smoothRadius; sxi <= smoothRadius; sxi++)
                        {
                            sum += terrain.Get(x + xi + sxi, z + zi + szi);
                        }
                    }
                    res[zi + adjustedRadius, xi + adjustedRadius] = sum / (n * n);
                }
            }
        }
        ...
        for (int zi = -adjustedRadius; zi <= adjustedRadius; zi++)
        {
            for (int xi = -adjustedRadius; xi <= adjustedRadius; xi++)
            {
                terrain.Set(x + xi, z + zi, res[zi + adjustedRadius, xi + adjustedRadius]);
            }
        }
    }
    ...
    private void ChooseDeformationChoice(int xLeft, int zLeft, int xRight, int zRight)
    {
        ...
        if (base.DeformationChoice == inputDeformation.useTerrainPrefabs)
        {
            youngMGround = YoungModulus;
            poissonR = PoissonRatio;
            applyBump = ActivateBump;
        ...
        if (base.DeformationChoice == inputDeformation.useUI)
        {
            applyCompression = ActivateToggleDef.isOn;
            applyBump = ActivateToggleBump.isOn;
            ...
            youngMGround = YoungSlider.value;
        ...
        for (int zi = -gridSize; zi <= gridSize; zi++)
        {
            for (int xi = -gridSize; xi <= gridSize; xi++)
            {
                ...
                if (LeftFootCollider.Raycast(upRayLeftFoot, out leftFootHit, raycastDistance))
                {
                    ...
                    if (showGridDebugLeft && IsLeftFootGrounded)
                ...
                if (RightFootCollider.Raycast(upRayRightFoot, out rightFootHit, raycastDistance))
                {
                    ...
                    if (showGridDebugRight && IsRightFootGrounded)
        ...
        foreach (Vector3 centerCell in cellsToStabilizeLeft)
        {
            ...
            for (int zi = -1 + (int)centerCell.z - nAround; zi <= 1 + (int)centerCell.z + nAround; zi++)
            {
                for (int xi = -1 + (int)centerCell.x - nAround; xi <= 1 + (int)centerCell.x + nAround; xi++)
                {
                    ...
                    //else if (heightMapLeftBoolWorld[(int)centerCell.z, (int)centerCell.x] == 2 && heightMapLeftBoolWorld[zi, xi] == 1)
                    //{
                    //    // First level
                    //    Vector3 ray = new Vector3(xi, terrain.Get(xi, zi), zi);
                    //    Vector3 rayWorld = terrain.Grid2World(ray);
            ...
            for (int zi = -1 + (int)centerCell.z - nAround; zi <= 1 + (int)centerCell.z + nAround; zi++)
            {
                for (int xi = -1 + (int)centerCell.x - nAround; xi <= 1 + (int)centerCell.x + nAround; xi++)
                {
                    ...
                    //else if (heightMapLeftBoolWorld[(int)centerCell.z, (int)centerCell.x] == 2 && heightMapLeftBoolWorld[zi, xi] == 1) // TEST
                    //{
                    //    // First level
                    //    Vector3 ray = new Vector3(xi, terrain.Get(xi, zi), zi);
                    //    Vector3 rayWorld = terrain.Grid2World(ray);
        ...
        foreach (Vector3 centerCell in cellsToStabilizeRight)
        {
            ...
            for (int zi = -1 + (int)centerCell.z - nAround; zi <= 1 + (int)centerCell.z + nAround; zi++)
            {
                for (int xi = -1 + (int)centerCell.x - nAround; xi <= 1 + (int)centerCell.x + nAround; xi++)
                {
                    ...
                    //else if (heightMapRightBoolWorld[(int)centerCell.z, (int)centerCell.x] == 2 && heightMapRightBoolWorld[zi, xi] == 1) // TEST
                    //{
                    //    // First level
                    //    Vector3 ray = new Vector3(xi, terrain.Get(xi, zi), zi);
                    //    Vector3 rayWorld = terrain.Grid2World(ray);
    ...
    #region Deformation Methods

    private void EstimateTargetDeformations()
    {
        // As for the area, we keep the maximum value
        oldHeightCellDisplacementYoungLeft = pressureStressLeft * (originalLengthZero / (youngMGround));
        if (oldHeightCellDisplacementYoungLeft >= heightCellDisplacementYoungLeft)
        {
            ...
            heightCellDisplacementYoungLeft = pressureStressLeft * (originalLengthZero / youngMGround);
        ...
        oldHeightCellDisplacementYoungRight = pressureStressRight * (originalLengthZero / (youngMGround));
        if (oldHeightCellDisplacementYoungRight >= heightCellDisplacementYoungRight)
        {
            // We use abs. value but for compression, the change in length is negative
            heightCellDisplacementYoungRight = pressureStressRight * (originalLengthZero / youngMGround);
    ...
    IEnumerator IncreaseTerrainLeft(float[,] heightMapLeft, int[,] heightMapLeftBool, float[,] weightsBumpLeftInit, float[,] weightsBumpLeft, int xLeft, int zLeft)
    {
        // 1. Apply frame-per-frame deformation ("displacement")
        for (int zi = -gridSize; zi <= gridSize; zi++)
        {
            for (int xi = -gridSize; xi <= gridSize; xi++)
            {
                ...
                if (!LeftFootCollider.Raycast(upRayLeftFoot, out leftFootHit, raycastDistance) && (heightMapLeftBool[zi + gridSize, xi + gridSize] == 2) && applyBump)
                {
                    if (applyAnisotropy)
                    {
    ...
    IEnumerator IncreaseTerrainRight(float[,] heightMapRight, int[,] heightMapRightBool, float[,] weightsBumpRightInit, float[,] weightsBumpRight, int xRight, int zRight)
    {
        // 1. Apply frame-per-frame deformation ("displacement")
        for (int zi = -gridSize; zi <= gridSize; zi++)
        {
            for (int xi = -gridSize; xi <= gridSize; xi++)
            {
                ...
                if (!RightFootCollider.Raycast(upRayRightFoot, out rightFootHit, raycastDistance) && (heightMapRightBool[zi + gridSize, xi + gridSize] == 2) && applyBump)
                {
                    if (applyAnisotropy && !modulateLeftFootOnly)
                    {
    ...
    IEnumerator StabilizationLeft(float[,] heightMapLeftWorld, List<StabilizationCell> cellsStabilizationDescendentLeft)
    {
        ...
        if (true)
        {
            foreach (StabilizationCell cell in cellsStabilizationDescendentLeft)
            {
                ...
                {
                    if (showStabilizationProcessLeft)
                    {
                        Debug.DrawRay(terrain.Grid2World(cell.PosCenterCell), Vector3.up, Color.green);
                        Debug.DrawRay(terrain.Grid2World(new Vector3(cell.Pos2DNeighbour.x, cell.Height, cell.Pos2DNeighbour.y)), Vector3.up * cell.Angle, Color.grey);
                    }
                ...
                else
                {
                    if (showStabilizationProcessLeft)
                    {
                        Debug.DrawRay(terrain.Grid2World(cell.PosCenterCell), Vector3.up, Color.red);
                        Debug.DrawRay(terrain.Grid2World(new Vector3(cell.Pos2DNeighbour.x, cell.Height, cell.Pos2DNeighbour.y)), Vector3.up * cell.Angle, Color.black);
                    }
    ...
    IEnumerator StabilizationRight(float[,] heightMapRightWorld, List<StabilizationCell> cellsStabilizationDescendentRight)
    {
        ...
        if (true)
        {
            foreach (StabilizationCell cell in cellsStabilizationDescendentRight)
            {
                ...
                {
                    if (showStabilizationProcessRight)
                    {
                        Debug.DrawRay(terrain.Grid2World(cell.PosCenterCell), Vector3.up, Color.green);
                        Debug.DrawRay(terrain.Grid2World(new Vector3(cell.Pos2DNeighbour.x, cell.Height, cell.Pos2DNeighbour.y)), Vector3.up * cell.Angle, Color.grey);
                    }
