...
/// </summary>
public class ExportVegetationMap : RunAbleThread
{
    // Property with 1D array  
    public byte[] VegetationBytes { get; set; }
    public byte[] InitialVegetationBytes { get; set; }
        ...
        using (RequestSocket client = new RequestSocket())
        {
            ...
            for (int i = 0; i < 1000000 && Running; i++)
            {
                ...
                */
                ...
                // Placeholder
                //LivingRatioBytes = new byte[1 * sizeof(float)];
                //if (LivingRatioBytes.Length > 0)
                //    client.SendFrame(LivingRatioBytes);

                if (VegetationCreator.LivingRatioBytes.Length > 0)
                    client.SendFrame(VegetationCreator.LivingRatioBytes);
                ...
                if (gotMessage)
                {
                    VegetationBytes = message;
                    Debug.Log("Received Vegetation " + message);

                    if (i == 5)
                    {
                        InitialVegetationBytes = message;
                    }
                }
            }
        }
